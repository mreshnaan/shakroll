...
const TimelineMobile: React.FC = () => {
  ...
  const goToPrevious = () => {
    setActiveStep((prevIndex) => (prevIndex === 0 ? steps.length - 1 : prevIndex - 1));
  };

  const goToNext = () => {
    setActiveStep((prevIndex) => (prevIndex === steps.length - 1 ? 0 : prevIndex + 1));
  };
  ...
  return (
    <div className="lg:hidden ">
      <div className="flex grid items-center justify-items-center grid-row-3">
            ...
            <CarouselContent>
              {steps.map((steps, index) => (
                <CarouselItem key={index}>
                  <div className="p-1 text-xl font-semibold text-cyan-100">{steps.date}</div>
                </CarouselItem>
              ))}
            </CarouselContent >
        ...
        <div className="relative flex flex-row items-center w-10/12">
            <div >
              <div
          ...
          <div className="absolute pl-5 w-lvw">
